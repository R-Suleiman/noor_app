...
export default function HomePage({ navigate }) {
  ...
  useEffect(() => {
    // Upgraded network requests to utilize unified axiosClient rules
    axiosClient.get("/tracks/trending")
  ...
  const handleStartListening = () => {
    if (tracks.length > 0) {
      ...
      document.dispatchEvent(new CustomEvent("play-first", { detail: { tracks } }));
    }
  };
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {artists.map((a) => (
              <div
                key={a.id}
                onClick={() => navigate("artist", { artistId: a.id })}
              ...
              >
                ...
                <p className="text-xs text-zinc-500 mt-2 font-medium">
                  {a._count?.tracks ?? 0} tracks · {a.user?._count?.followers ?? 0} followers
                </p>
              </div>
            ))}
          </div>
        ...
        <div className="sticky top-8 bg-gradient-to-b from-zinc-900/60 to-zinc-900/20 border border-white/5 rounded-2xl p-6 shadow-xl min-h-[450px] flex flex-col justify-between overflow-hidden">
          {current ? (
            <div className="flex flex-col h-full animate-[fadeIn_0.3s_ease-out]">
              ...
              <div className={`aspect-square w-full rounded-xl flex items-center justify-center shadow-lg relative group mb-5 overflow-hidden ${trackBg(current)}`}>
                {current.album?.coverUrl ? (
                  <img src={current.album.coverUrl} alt={current.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                ) : (
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  ...
                  >
                    <i className={`ti ${playing ? "ti-player-pause-filled" : "ti-player-play-filled"} text-xl`} />
                  </button>
                </div>
              </div>
              ...
              <div className="mb-4">
                <h3 className="text-lg font-bold text-zinc-100 truncate tracking-tight">{current.title}</h3>
                <p className="text-sm text-zinc-400 truncate mt-0.5 font-medium">{current.artist?.name ?? "Unknown Speaker"}</p>
              </div>
              ...
              <div className="bg-zinc-800/40 border border-white/5 rounded-xl p-4 flex-grow flex flex-col justify-center">
                <p className="text-xs uppercase font-bold tracking-wider text-zinc-500 mb-1">Speaker Meta Context</p>
                ...
                {current.genre && (
                  <div className="mt-3 flex items-center gap-1.5 text-xs font-semibold text-emerald-400 bg-emerald-500/10 max-w-max px-2.5 py-1 rounded-md">
                    ...
                    <span className="capitalize">{current.genre.toLowerCase()} Category</span>
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col items-center justify-center flex-grow text-center p-6 my-auto">
              ...
              <h4 className="text-sm font-bold text-zinc-300">No Track Selected</h4>
              <p className="text-xs text-zinc-500 max-w-[200px] mt-1 leading-normal">
                Double-click any row item or choose an artist to spin up an audio streaming engine.
              </p>
            </div>
          )}
        </div>
