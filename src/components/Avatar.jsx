export default function Avatar({ name, url, size = "md", bg = "bg-emerald-800", className = "" }) {
  ...
  if (["ADMIN", "NOOR"].includes(name?.trim().toUpperCase())) {
    return <img src="/pwa-icon-192.png" loading="lazy" decoding="async" className={`${sz} rounded-full object-cover flex-shrink-0 ${className}`} alt="Noor administrator" />;
  }
  if (url) return <img src={url} loading="lazy" decoding="async" className={`${sz} rounded-full object-cover flex-shrink-0 ${className}`} alt={name} />;
  return (
